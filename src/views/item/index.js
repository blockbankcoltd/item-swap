      ...
      .then(async (res) => {
        if (res.data.orders.length > 0) {
          ...
          let minimumAmount = Math.min(
            ...activeTrades.map((item) => parseFloat(item.makePrice)),
          ).toFixed(20);
          let result = res.data.orders.find(
            (item) => parseFloat(item.makePrice) == minimumAmount,
          );
          console.log(result);
          setOrders([result]);
        }
                ...
                {orders.length > 0 && account !== itemData.owner_of ? (
                  <div className="d-sm-flex justify-content-between align-items-center">
                    ...
                    <div className="d-flex justify-content-around align-items-center">
                      ...
                      <button
                        className="primary-btn text-nowrap mx-2 w-100"
                        onClick={() => handleBuy(orders[0].makePrice)}
                      >
                        Buy
                      </button>
                      {/* Place a bid section */}
